...
class Followgrass(Node):

    def __init__(self):
        super().__init__('follow_grass')
        self.subscription = self.create_subscription(Point, '/detected_grass', self.listener_callback, 10)
    ...
    def listener_callback(self, msg):
        ...
        self.lastrcvtime = time.time()  # self.get_logger().info('Received: {} {}'.format(msg.x, msg.y))


def main(args=None):
    rclpy.init(args=args)
    follow_grass = Followgrass()
    rclpy.spin(follow_grass)
    follow_grass.destroy_node()
    rclpy.shutdown()
